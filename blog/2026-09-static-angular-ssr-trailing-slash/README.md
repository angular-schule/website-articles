...
It looks great in the address bar, but technically, for search engine optimization, it's a nightmare.
...
The option is made for static builds, by design.
...
If your build deploys a server (`"outputMode": "server"`, or `ssr` without `outputMode`), the option is not considered: you get a warning, and the pages are written to `<route>/index.html` as before.
...
In this case, builder and pull request keep `index/index.html` for that route and show a warning.
