...
With a server, there is nothing to fix anyway: the Node.js server generated by Angular serves static files with `redirect: false`, so `/blog` is never redirected to `/blog/` in the first place.
...
That's a hack.
It relies on internal APIs, which is why it supports Angular 22 only.
