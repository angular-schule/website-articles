...
Ein Formularmodell kann zum Beispiel so aussehen:
...
Über die Property `value` bzw. die Methode `getRawValue()` können wir den Wert des Formulars auslesen, um damit weiterzuarbeiten.
...
Das Angular-Team hat sich dieses Problems nun angenommen: Ab Angular 14 ist das Formularmodell korrekt typisiert!

## Typed Forms

Seit Angular 14 sind alle Bausteine von Reactive Forms typisiert.
...
Um dieses Verhalten nicht zu brechen, ist `null` im Typ immer enthalten.

Um das zu umgehen, können wir bei der Initialisierung des FormControls die neue Option `initialValueIsDefault` setzen.
Das ändert das Reset-Verhalten des Controls: Beim Zurücksetzen wird nicht `null` verwendet, sondern der ursprünglich definierte Startwert.
...
Entwickeln Sie ein Formular "auf der grünen Wiese", empfehlen wir Ihnen, die Option für jedes Control zu setzen.
...
Das bedeutet, dass alle Felder optional markiert sind, also auch den Typ `undefined` inkludieren:
...
Es ist übrigens empfehlenswert, die `FormGroup` immer direkt bei der Deklaration des Propertys zu initialisieren.
...
Wenn wir ein dynamisches Formular entwickeln, das zur Laufzeit seine Struktur ändert, können wir uns nicht auf die Typisierung verlassen.
...
In diesem Fall müssen wir auf die untypisierte Variante `UntypedFormControl` oder auf den neuen `FormRecord` zurückgreifen.

## FormRecord

Um das Problem mit `addControl()` in einer `FormGroup` zu lösen, wurde ein neuer Baustein `FormRecord` eingeführt.
Inhaltlich funktioniert dieses Objekt wie eine `FormGroup`, es besitzen jedoch alle Felder denselben Typ.
Das ist besonders dann sinnvoll, wenn Controls zur Laufzeit hinzugefügt/entfernt werden sollen:
...
checkboxGroup.addControl('acceptCOC', new FormControl(false)); // OK!
```

Bei der Typisierung des Values kommt TypeScript allerdings wieder an seine Grenzen: Die Namen/Keys der Felder sind nicht bekannt, sondern generisch mit `string` definiert:
...
Das Angular-Team hat viel Zeit und Energie in neue Features gesteckt und damit die lang ersehnten Wünsche der Community berücksichtigt.
Die neuen typisierten Bausteine für Reactive Forms bringen ein neues Level an Typsicherheit in Angular-Anwendungen.
